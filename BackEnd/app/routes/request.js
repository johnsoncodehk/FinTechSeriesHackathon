...
router.post('/:rid', function (req, res) {
	...
	pool.getConnection(function(err, connection) {
		...
		sql = 'SELECT * FROM ON_AIR_REQUEST WHERE REQUEST_ID IN (?) AND END_DATE > NOW()';
		var arg = [rid];
		...
		connection.query(sql, arg, function(err, rows) {
			...
			var sql = 'INSERT INTO `JOIN_REQUEST` (`REQUEST_ID`, `UID`, `CREATE_DATE`) \
				VALUES (?, ?, NOW())';
			var arg = [rid, req.session.user.ID];
